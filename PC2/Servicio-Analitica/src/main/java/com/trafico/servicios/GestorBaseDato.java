 ...
 */
public class GestorBaseDato implements Runnable {
     ...
     * Maneja valores null para sensores parciales.
     */
    private void procesarYGuardarDato(String mensaje) {
        try {
            ...
            // Obtener valores opcionalmente, con null si no existen
            Double velocidadPromedio = null;
            Integer densidad = null;
            Integer cola = null;

            Object velObj = datos.get("velocidad_promedio");
            if (velObj != null) {
                velocidadPromedio = ((Number) velObj).doubleValue();
            }

            Object denObj = datos.get("densidad");
            if (denObj != null) {
                densidad = ((Number) denObj).intValue();
            }

            Object colaObj = datos.get("cola");
            if (colaObj != null) {
                cola = ((Number) colaObj).intValue();
            }
    ...
    private void guardarEnBD(String interseccion, String estado, String timestamp,
                           Double velocidadPromedio, Integer densidad, Integer cola) {
        ...
        try {
            ...
            intentarFallbackPC2(interseccion, estado, timestamp, velocidadPromedio, densidad, cola);
        } finally {
            if (conn != null) {
                try {
                    conn.close();
     ...
     * Maneja valores null para sensores parciales.
     */
    private void intentarFallbackPC2(String interseccion, String estado, String timestamp,
                                     Double velocidadPromedio, Integer densidad, Integer cola) {
        Connection conn = null;
        try {
